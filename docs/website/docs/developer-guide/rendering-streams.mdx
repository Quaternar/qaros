...
The **render sender** is how your application's pixels reach the room. You create one sender per target, then drive a simple loop: *begin frame → render into the provided textures → show frame*. QarOS takes care of encoding, transport, and — via the [mixer](/docs/user-guide/the-mixer) — warping and compositing your content into every viewer's personal view.
...
```csharp
// TODO: C# sample pending publication of the qar-streaming-sharp render
// sender bindings in this repository. Flow mirrors the C version.
```
...
- **Target peer** — a sender streams to one peer (get peer IDs from [peer enumeration](/docs/developer-guide/onboarding-and-sessions) or your own invite result).
...
:::note TODO
Link the compiled `cpu_rendering_visualizer.c` tutorial page here once it is updated to onboarding-based session acquisition; extend it with Doxygen snippet sections for the frame loop and a D3D11 variant example.
:::

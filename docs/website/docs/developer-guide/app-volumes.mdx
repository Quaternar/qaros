...
An **app volume** is your application's home in the room: a named 3D box that holds your rendered content, defines where the mixer composites your stream, and mediates how users grab, move, and scale your content. Read the conceptual model first: [The QAROS World](/docs/user-guide/qaros-world#app-volumes) and [Coordinate Systems](/docs/developer-guide/coordinate-systems).
...
```csharp
var options = new AppVolumeOptions
{
    CommonName  = "assembly-view.myapp.example.com",
    DisplayName = "Assembly View",

    // Pose is the cuboid CENTER, room space.
    Pose = new Pose { Position = new Vec3(0f, 1.2f, 0f) },
    Size = new AppVolumeSize
    {
        WidthMeters  = 1.0f,
        LengthMeters = 1.0f,
        HeightMeters = 0.8f,
    ...
    AppPose  = new Pose(),  // app origin inside the box
    AppScale = 0.01f,       // 1 app-meter -> 1 room-cm: a 100 m model fits the box
};

Result<AppVolumeId> created = session.AppVolumes.GetOrCreate(options);
AppVolumeId volumeId = created.ValueOrThrow();
```
...
To pin a volume to a geographic pose (ECEF WGS84 — see [Coordinate Systems](/docs/developer-guide/coordinate-systems#world-space--pinning-the-room-to-the-earth)):
...
- Source: `qaros/qar-streaming-c/examples/app_volume_management.c`
- Example walkthrough: [App Volume Management](/docs/developer-guide/tutorials/c/app-volume-management)

...
// Onboard on OnboardingSessionNotFound, re-prompt on a pairing error.
...
{
    var who = new PeerPresentationOptions
    {
        DisplayName = "My CAD Tool",
        AppVersion = "2.4.0"
    };

    if (savedId != OnboardingId.Default)
    {
        var rejoin = runtime.Rejoin(new RejoinOptions(savedId).WithPresentation(who));
        if (rejoin.IsSuccess)
    ...
    var onboard = runtime.Onboard(
        new OnboardOptions
    {
        Presentation = who,
    }.UseCode(codeFromUser!));
    if (!onboard.IsSuccess)
        return Result<Session>.Failure(onboard.Info);
    ...
    savedId = onboard.Value.OnboardingId;       // persist before returning
    return Result<Session>.Success(onboard.Info, onboard.Value.Session);
}
```

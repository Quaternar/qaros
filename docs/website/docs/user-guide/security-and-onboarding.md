...
![Security layers](/img/diagrams/security-layers.svg)
...
**For application developers:** the entire stack above is hidden behind three C API calls — *onboard with code*, *rejoin*, and *forget*. Your app persists one opaque onboarding ID and never touches keys or certificates. See [Onboarding and Sessions](/docs/developer-guide/onboarding-and-sessions).

## Current limitations (honest list)

These are known, documented limitations of the current implementation:

- Private keys and CA secrets are stored on disk protected by OS permissions, not yet by encrypted storage / hardware keystores.
- Discovery (Layer 0) has no TLS — by design it carries no secrets, but it can be noisy on shared networks.
- A certificate that expires while a device is offline cannot be recovered; the device re-onboards.
- The CA bootstrap is currently Windows-only.
- The revocation blacklist is local to a Hub (not federated).

:::note TODO
Keep this list in sync with the security roadmap; several items (encrypted secret storage, discovery hardening) are planned. This page intentionally documents the *model* — a deep-dive threat-model page for security auditors (assumptions, per-layer attack analysis, storage layout) should be added as a separate advanced page.
:::

...
![Coordinate spaces](/img/diagrams/coordinate-spaces.svg)
...
The room origin itself is established by the underlying XR runtime and the platform's relocalization — it is not something applications set through the API.

:::note TODO
Document precisely how multiple headsets co-locate to the same room origin (the relocalization/QR-anchor workflow per device type), and state the room-space handedness/axis convention normatively in the C API headers rather than by OpenXR inheritance. Both are currently implementation behavior, not documented API contract.
:::
